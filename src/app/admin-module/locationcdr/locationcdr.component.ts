...
@Component({
  selector: 'app-locationcdr',
  templateUrl: './locationcdr.component.html',
  styleUrls: ['./locationcdr.component.css']
})
export class LocationcdrComponent {
  locationcdr:FormGroup;
...
dtoptions: DataTables.Settings = {};
  constructor(public service:CdrserviceService,private fb: FormBuilder,public session:SessionstorageService,public route:Router){
    this.locationcdr = this.fb.group({
  ...
  generateCDRs(){
    console.log("hi")
    const n=this.locationcdr.get('n')?.value;
    console.log(n);
    // this.service.getcdr(n).subscribe((pro)=>{this.productlist=pro});
    this.searchlist=this.service.getlocationcdr(n).subscribe((e: any)=>{if(Array.isArray(e))
      {
        console.log(e);
        this.searchlist=e;
